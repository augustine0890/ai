...
```bash
cp config/input.example.json config/input.json
```

Then fill in `config/input.json` — see the next section for every field.

---

## Configuring input.json

`config/input.json` is the single config file the downloader reads at startup. It is gitignored so your tokens never leave your machine.
...
10. Paste it as the `authorization_token` value in `config/input.json`.
...
8. Paste it as the `policy_key` value in `config/input.json`.
...
```bash
uv run python -m dds.datascience365.main
```

`main.py` scrapes the `/courses` listing page, extracts all course links, and downloads each one in sequence. If the listing page is login-gated, it falls back to `course_url` / `course_urls` in `config/input.json`.
...
```bash
uv run python -m dds.datascience365.compress_courses
```
...
| Command | Action |
|---|---|
| `uv run python -m dds.datascience365.compress_courses` | Compress all pending courses |
| `uv run python -m dds.datascience365.compress_courses --batch 5` | Compress only the next 5 pending courses |
| `uv run python -m dds.datascience365.compress_courses "SQL"` | Compress only courses matching "SQL" |
| `uv run python -m dds.datascience365.compress_courses --list` | List all courses with status, file counts, and sizes |
| `uv run python -m dds.datascience365.compress_courses --transcode` | Transcode videos to H.265, then compress all pending courses |
| `uv run python -m dds.datascience365.compress_courses --transcode "SQL"` | Transcode + compress matching course only |
| `uv run python -m dds.datascience365.compress_courses --transcode --crf 28` | Transcode with custom CRF value (default: 26) |
| `uv run python -m dds.datascience365.compress_courses --batch 3 --transcode` | Transcode + compress next 3 pending courses |

#### Typical batch workflow (limited storage)

```bash
# First run: compress 5 courses
uv run python -m dds.datascience365.compress_courses --batch 5
...
uv run python -m dds.datascience365.compress_courses --list
```
...
Set `course_url` in `config/input.json`, then:
...
⚠️  [Course API] Token expired or invalid: ...
  ...
  → Open  /path/to/dds/config/input.json
...
2. Open `config/input.json` in your editor and replace `authorization_token` with the new value. Save the file.
3. Press **Enter** in the terminal.

The downloader reads the new token from `config/input.json`, updates its in-memory state, and **resumes from where it left off** — no restart, no re-downloading.
...
Every run writes structured logs to `output/trace.jsonl` (in the `dds/` folder) and prints them to the terminal.
...
To debug a failed run, paste the relevant portion of `output/trace.jsonl` into your LLM conversation. The most useful approach:

1. Open `output/trace.jsonl`.
2. Find the `session` value from the run you want to debug (it appears in every line).
3. Filter lines by that session:
   ```bash
   grep '"session": "a1b2c3d4"' output/trace.jsonl
   ```
...
**Fix:** Set `course_url` or `course_urls` directly in `config/input.json`.
...
**Fix:** [Get a fresh token](#how-to-get-authorization_token) and update `config/input.json`. If this happens mid-run, the downloader will pause and prompt you — see [Token expiry mid-run](#token-expiry-mid-run).
...
**Fix:** The downloader automatically tries `/course/text/{asset_id}` and then lecture fallback endpoints. If all return nothing, the asset is logged as `lesson_content_missing`. Check your `output/trace.jsonl` for those events.
...
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌──────────────────────────────┐   │
│  │  config/input.json │───▶│   main.py    │───▶│   download_single_course.py  │   │
│  │  (config)   │    │ orchestrator │    │      download engine         │   │
│  └─────────────┘    └──────┬───────┘    └──────────────┬───────────────┘   │
│                            │                           │                   │
│  ┌─────────────┐           │            ┌──────────────┴───────────────┐   │
│  │  logger.py  │◀──────────┴────────────│   course_model.py            │   │
│  │ output/trace.jsonl │                        │   video_model.py             │   │
...
```
  ...
  │  STARTUP                                                         │
  │                                                                  │
  │  config/input.json ──▶ load_input_data()                               │
...
```
  ...
  │                                                          │
  │  user saves new token to config/input.json in their editor     │
  │  user presses Enter                                      │
  │                                                          │
  │  reads config/input.json ──▶ update_auth_token(new_token)      │
  │    sets module-level _CURRENT_AUTH_TOKEN                 │
  │    re-writes config/input.json with new value                   │
  └──────────────────────────────────────────────────────────┘
        │
        ▼
...
- `config/input.json` is gitignored — never commit real tokens.
- Network or API changes upstream may break scraping or download behavior; check `output/trace.jsonl` for details.
